...
def evaluate_repeat_baseline(test_loader: DataLoader) -> dict:
    ...
    results = {"mse": [], "mae": [], "pearson": [], "rpeak_f1": [], "zero_peaks": [], "total_samples": []}
    ...
    for batch in test_loader:
        ...
        results["zero_peaks"].append(rhythm_m["zero_peak_count"])
        results["total_samples"].append(rhythm_m["total_samples"])

    tot_zero = int(sum(results["zero_peaks"]))
    tot_samples = int(sum(results["total_samples"]))

    res = {k: float(np.mean(v)) for k, v in results.items() if k not in ["zero_peaks", "total_samples"]}
    res["zero_peaks_summary"] = f"{tot_zero} out of {tot_samples} samples ({tot_zero/max(1, tot_samples)*100:.1f}%) had 0 detected R-peaks"
    return res
...
def main():
    ...
    print(f"  Zero R-peaks status: {base_m['zero_peaks_summary']}")
    for k, v in base_m.items():
        if k != "zero_peaks_summary":
    ...
    if os.path.exists(args.checkpoint):
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                ensemble_mean = samples_reshaped.mean(dim=0)
                ...
                model_results["zero_peaks"].append(rhythm_m["zero_peak_count"])
                model_results["total_samples"].append(rhythm_m["total_samples"])

        tot_zero = int(sum(model_results["zero_peaks"]))
        tot_samples = int(sum(model_results["total_samples"]))

        avg_res = {k: float(np.mean(v)) for k, v in model_results.items() if k not in ["zero_peaks", "total_samples"]}
        print("\n--- Latent SDE Model Evaluation Results ---")
        print(f"  Zero R-peaks status: {tot_zero} out of {tot_samples} samples ({tot_zero/max(1, tot_samples)*100:.1f}%) had 0 detected R-peaks")
